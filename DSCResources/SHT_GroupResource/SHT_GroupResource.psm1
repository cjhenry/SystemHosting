<#
 # SHT_GroupResource - A modified version of the MSFT_GroupResource DSC resource containing the following bugfix:
 #   https://connect.microsoft.com/PowerShell/feedbackdetail/view/951681/desired-state-configuration-group-resource
 #
 #
 # Authored by: Martin T. Nielsen - mni@systemhosting.dk / Dennis Rye - dry@systemhosting.dk
 #>
...
function Set-TargetResource
{
    ...
    try
    {
        ...
        elseif ($env:USERDOMAIN -ne $env:COMPUTERNAME) { # If computer is domain joined, we can probably look up AD objects without credentials
            $credentialPrincipalContext = New-Object System.DirectoryServices.AccountManagement.PrincipalContext -ArgumentList ([System.DirectoryServices.AccountManagement.ContextType]::Domain)        
        }
